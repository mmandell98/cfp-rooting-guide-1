...
const store = {
  get<T>(k: string, f: T): T { try { const v = localStorage.getItem(k); return v ? JSON.parse(v) : f; } catch { return f; } },
...
export default function Page() {
  ...
  const [objective, setObjective] = useState(store.get('cfp.objective', 'Make CFP'));
  const [chaos, setChaos] = useState(store.get('cfp.chaos', 70));
  const [sos, setSos] = useState(store.get('cfp.sos', 40));
  const [confBias, setConfBias] = useState(store.get('cfp.confBias', 60));

  const [year, setYear] = useState<number>(new Date().getFullYear());
  const [week, setWeek] = useState<string>('');
  const [seasonType, setSeasonType] = useState<'regular'|'postseason'|'both'>('regular');
  const [poll, setPoll] = useState<'Playoff Committee'|'AP Top 25'>(store.get('cfp.poll', 'AP Top 25'));
  ...
  const opponentsPlayed = useMemo(() => opponentsText.split(',').map(s => s.trim()).filter(Boolean), [opponentsText]);
  ...
  async function loadData() {
    ...
    setGames(((gRes.games || []) as any[]).map(g => ({ home: g.home, away: g.away, conference: g.conference, week: g.week })));
    setNote(rRes.note);
    const map: Record<string, { logos?: string[] }> = {};
    (tRes.teams || []).forEach((t: any) => { map[t.school] = { logos: t.logos }; });
    setTeams(map);

    const wk = (wRes.weeks || []) as { week: number, firstGameStart: string, lastGameStart: string }[];
    const normalized = wk.map(w => ({ week: w.week, start: (w as any).firstGameStart || (w as any)['first_game_start'] || '', end: (w as any).lastGameStart || (w as any)['last_game_start'] || '' }));
    setWeeks(normalized);
    const now = new Date().toISOString();
    const current = normalized.find(w => (w.start && w.end) ? (w.start <= now && now <= w.end) : false);
    setCurrentWeek(current?.week ?? null);
  }
  useEffect(() => { loadData(); }, [year, week, seasonType, poll]);

  const guide = useMemo(() => computeGuide({
    favorite, objective, chaosLevel: chaos, sosWeight: sos, confBias,
    schedule: games, rankings, opponentsPlayed,
    ...
    <div className="min-h-screen">
      ...
      <main style={{ maxWidth: 1100, margin: '0 auto', padding: 16, display: 'grid', gap: 24, gridTemplateColumns: '1fr', alignItems: 'start' }}>
        <section style={{ border: '1px solid #eee', borderRadius: 16, padding: 16 }}>
          ...
          <input list="teams" value={favorite} onChange={e=>setFavorite(e.target.value)} placeholder="e.g., UVA" style={{ width:'100%', border:'1px solid #ddd', borderRadius:12, padding:'8px 10px', marginTop:4, marginBottom:8 }} />
          <datalist id="teams">{allTeams.map(t => <option key={t} value={t} />)}</datalist>

          <label style={{ fontSize: 14, fontWeight: 600 }}>Objective</label>
          <select value={objective} onChange={e=>setObjective(e.target.value)} style={{ width:'100%', border:'1px solid #ddd', borderRadius:12, padding:'8px 10px', marginTop:4, marginBottom:8 }}>
            {['Make CFP','Top 12 Seed','Win Conference'].map(o => <option key={o} value={o}>{o}</option>)}
          </select>
          ...
          <div style={{ display:'flex', gap:8, margin:'4px 0 8px' }}>
            {(['Playoff Committee','AP Top 25'] as const).map(p => (
              <button key={p} onClick={()=>setPoll(p)} style={{ padding:'6px 10px', border:'1px solid #ddd', borderRadius:999, background: poll===p? '#111' : '#fff', color: poll===p? '#fff':'#111', fontSize:13 }}>
                {p === 'Playoff Committee' ? 'CFP' : 'AP'}
              </button>
            ))}
          </div>
          {note && <div style={{ fontSize: 12, color:'#555', background:'#fff7ed', border:'1px solid #ffedd5', padding:'6px 10px', borderRadius:12 }}>ℹ️ {note}</div>}
          ...
          <div style={{ marginTop: 8 }}>
            ...
            <div style={{ display:'grid', gridTemplateColumns:'1fr auto auto', gap:8, alignItems:'center', marginTop:4 }}>
              <select value={week} onChange={e=>setWeek(e.target.value)} style={{ width:'100%', border:'1px solid #ddd', borderRadius:12, padding:'8px 10px' }}>
                <option value="">All weeks</option>
                {futureWeeks.map(w => (
                  <option key={w.week} value={String(w.week)}>
                    Week {w.week}{w.start && w.end ? ` (${fmtDate(w.start)} — ${fmtDate(w.end)})` : ''}
                  </option>
                ))}
                {weeks.filter(w=>!futureWeeks.find(f=>f.week===w.week)).map(w => (
                  <option key={`past-${w.week}`} value={String(w.week)}>
                    Week {w.week}{w.start && w.end ? ` (${fmtDate(w.start)} — ${fmtDate(w.end)})` : ''} — past
                  </option>
                ))}
              </select>
          ...
          <div style={{ marginTop: 12 }}>
            ...
            <input value={opponentsText} onChange={e=>setOpponentsText(e.target.value)} placeholder="UNC, Louisville, Notre Dame" style={{ width:'100%', border:'1px solid #ddd', borderRadius:12, padding:'8px 10px', marginTop:4 }} />
          </div>
        </section>
        ...
        <section style={{ border: '1px solid #eee', borderRadius: 16, padding: 16 }}>
          <div style={{ display:'flex', alignItems:'center', justifyContent:'space-between' }}>
            ...
            <div style={{ fontSize: 13, color:'#666' }}>Configured for <b>{favorite}</b> · Goal: <b>{objective}</b> · Poll: <b>{poll==='Playoff Committee'?'CFP':'AP'}</b></div>
          </div>
                ...
                <div style={{ display:'flex', alignItems:'flex-start', gap:12 }}>
                  <div style={{ flex:1 }}>
                    <div style={{ display:'flex', gap:8, alignItems:'center', flexWrap:'wrap' }}>
                      ...
                      <span style={{ marginLeft:'auto', fontSize:11, background:'#f6f7f8', border:'1px solid #eee', padding:'4px 8px', borderRadius:999 }}>Week {g.week}</span>
                    </div>
                    <div style={{ marginTop:8 }}>
          ...
          {!guide.length && <div style={{ fontSize:14, color:'#666' }}>No games found — pick a different week or season type.</div>}
        </section>
      </main>
    </div>
  );
  ...
  return (
    <div title={name} style={{ display:'inline-flex', alignItems:'center' }}>
      {logo ? (
        ...
        <div style={{ width:size, height:size, background:bg, color:'#fff', borderRadius:999, display:'grid', placeItems:'center', fontWeight:600, boxShadow:'0 1px 2px rgba(0,0,0,.08)' }}>{initials}</div>
      )}
